...
void wordCHANGE(uint8_t* word){
    uint8_t test[WORD_LEN];
    memcpy(test, word, WORD_LEN);
    uint8_t temp = word[0];
    ...
    for(int i = 0;i < WORD_LEN; i++){
        word[i] = word[i] ^ test[i];
    }
}

void Create_roundkey(uint8_t* before_key, uint8_t* roundkey){
    uint8_t bk[AES_BLOCK_SIZE];
    memcpy(bk, before_key, AES_BLOCK_SIZE);
    for(int i = 0; i < 4; i++){
        wordCHANGE(bk + (i * WORD_LEN));
    }
    memcpy(roundkey, bk, AES_BLOCK_SIZE);
}
